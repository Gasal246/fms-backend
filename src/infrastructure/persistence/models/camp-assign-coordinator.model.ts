...
const schema: Schema = new mongoose.Schema(
  {
    camp_id: { type: mongoose.Schema.Types.ObjectId, ref: "camp", required: true },
...
// Compound unique index: one coordinator can be assigned to a camp only once
schema.index({ camp_id: 1, coordinator_id: 1 }, { unique: true });

const CampAssignCoordinator = mongoose.model<ICampAssignCoordinator>(
  "camp_assign_coordinator",
  schema
);
